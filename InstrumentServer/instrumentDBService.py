...
def addModelOptions(connection, model_and_options, id):
    ...
    with connection.cursor() as cursor:
        ...
        for column_name in column_names:
            if column_name[0] in model_and_options.keys():
                if column_name[0] == 'models':
                    columns.append('models')
                    models = '{' + ','.join(model_and_options['models'].keys()) + '}'
                    values.append(models)
                    columns.append('model_ids')
                    model_ids = '{' + ','.join(model_and_options['models'].values()) + '}'
                    values.append(model_ids)
                elif column_name[0] == 'options':
                    columns.append('options')
                    options = '{' + ','.join(model_and_options['options'].keys()) + '}'
                    values.append(options)
                    columns.append('option_ids')
                    option_ids = '{' + ','.join(model_and_options['options'].values()) + '}'
                    values.append(option_ids)

                elif model_and_options[column_name[0]]:
                    columns.append(column_name[0])
                    values.append(model_and_options[column_name[0]])
...
def addVisa(connection, visa: dict, id):
    ...
    with connection.cursor() as cursor:
        ...
        for column_name in column_names:
            if column_name[0] in visa.keys():
                if visa[column_name[0]]:
                    columns.append(column_name[0])
                    values.append(visa[column_name[0]])
